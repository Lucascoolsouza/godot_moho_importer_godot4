...
func create_chain(ik: Node2D, root: Node2D, tip: Node2D) -> void:
	...
	while root.is_ancestor_of(current_bone) or current_bone == root:
...
func draw(canvas: CanvasItem) -> void:
	...
	for point in _points:
		...
		last_point = point
	for point in _points:
		canvas.draw_circle(point.position, 10.0, Color.YELLOW_GREEN)
...
func solve_interactive(
		...
		node: Node,
		signal_name: String,
		progress_func: Callable = Callable()
) -> void:
	# Update all point position
	for idx in _points.size():
		var chain_item : Skeleton2DIKChainPoint = _points[idx]
...
func _update_progress(progress: Callable, text: String) -> void:
