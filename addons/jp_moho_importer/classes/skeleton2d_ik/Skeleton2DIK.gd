...
@export var preference : int = Skeleton2DIKChain.Preference.NONE: set = _set_preference # (Skeleton2DIKChain.Preference)
@export var target_node_path : NodePath: set = _set_target_node_path
# Global position target
@export var target : Vector2 = Vector2.INF: set = _set_target
@export var root_path : NodePath: set = _set_root_path
@export var tip_path := NodePath("."): set = _set_tip_path
...
func _draw():
	if Engine.is_editor_hint() and is_instance_valid(ik_chain):
		ik_chain.draw(self)
