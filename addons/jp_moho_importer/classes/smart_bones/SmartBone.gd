...
@export var handle = 0.5: set = _set_handle
# Array of SmartBoneAction type resources
@export var actions : Array # (Array, Resource)
...
@export var bone_scale : Vector2 = Vector2.ONE: set = _set_bone_scale
@export var flip_h : bool = false: set = _set_flip_h
@export var flip_v : bool = false: set = _set_flip_v

@export var flip_h_animation : bool = false: set = _set_flip_h_animation

# If true, this bone's rotation is independent from its parent transform.
# Angle is saved relative to it's parent rest angle.
@export var independent_angle : bool = false: set = _set_independent_angle
@export var parent_rest_angle : float: set = _set_parent_rest_angle
...
var parent_angle : float = 0.0: set = _set_parent_angle
...
func is_class(p_class: String) -> bool:
	return p_class == get_class() or super.is_class(p_class)
...
func _draw():
	if _should_draw_actions_gizmos():
		...
		for idx in actions.size():
			...
			draw_string(DEBUG_FONT, text_position, text, HORIZONTAL_ALIGNMENT_LEFT, 40, DEBUG_FONT.size, Color.WHITE)
...
func _has_actions() -> bool:
	return (not actions.is_empty()) and (get_child_count() >= actions.size())
