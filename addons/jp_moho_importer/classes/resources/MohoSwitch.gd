...
func _get_animated_properties() -> Array:
	return ["position", "scale", "rotation_z", "flip_h", "flip_v", "switch_keys"]
...
func initialize_switch_layer(switch_layer: SwitchLayer, verbose: bool = false) -> void:
	...
	node.name = name
	node.position = _get_property("position")
...
func _format_property(key: String, raw_value):
	var value = super._format_property(key, raw_value)
...
func _connect_to_signal(initial_key: String) -> void:
	...
	if not editor_tree.is_connected("tree_changed", Callable(self, "_on_switch_node_tree_changed")):
		editor_tree.connect("tree_changed", Callable(self, "_on_switch_node_tree_changed").bind(initial_key))
...
func _disconnect_all_signals() -> void:
	for connection in get_incoming_connections():
		...
		source.disconnect(connection.signal_name, Callable(self, connection.method_name))
...
func _on_switch_node_tree_changed(initial_key: String) -> void:
	...
	if not proper_key.is_empty():
		node.key = initial_key
		node.get_tree().disconnect("tree_changed", Callable(self, "_on_switch_node_tree_changed"))
