...
func _get_animated_properties() -> Array:
	return ["position", "scale", "rotation_z", "flip_h", "flip_v"]
...
func initialize_sprite(sprite_node: Sprite2D, verbose: bool = false) -> void:
	node = sprite_node
	anchor = node.get_parent() as Node2D
	
	node.name = name
	var position = _get_property("position")
	if position:
...
func get_image_filename() -> String:
	...
	if _is_vector():
		...
		var has_image = not points.is_empty()
		if has_image:
			image_filename = name
	else:
