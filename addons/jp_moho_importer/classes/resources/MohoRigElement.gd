...
Adding track for %s %s property : %s"
	...
	var track_idx : int = animation.find_track(track_path, Animation.TYPE_VALUE)
	...
	for idx in _get_frame_count(animation_data):
		...
		if verbose:
			print("| Added keyframe for %s %s at %0.2fs with value: %s" % [get_class(), name, time, value])
...
func _get_proper_key_idx(
	...
	var key_idx = animation.track_find_key(track_idx, time, Animation.FIND_MODE_APPROX)
	return key_idx
