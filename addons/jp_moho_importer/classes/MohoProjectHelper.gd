...
func set_images_from_folder() -> Array:
	...
	var sprites_found = 0
	var sprites_missing := []
	var icon_placeholder : Texture2D = load(ICON_PLACEHOLDER_PATH)
	if _verbose:
		print("\nSetting images on sprites")
	for idx in sprites.size():
		var sprite : MohoSprite = sprites[idx]
		
		var sprite_node : Sprite2D = sprite.node
	...
	if not sprites_missing.is_empty():
...
func _initialize_bone(
	...
	if not bone.is_connected("action_register", Callable(self, "_on_rig_element_action_register")):
		bone.connect("action_register", Callable(self, "_on_rig_element_action_register"))
...
func _initialize_sprite(
	...
	if not sprite.is_connected("action_register", Callable(self, "_on_rig_element_action_register")):
		sprite.connect("action_register", Callable(self, "_on_rig_element_action_register"))
...
func _initialize_group(
	...
	group.fps = _fps
	groups.append(group)
	if not group.is_connected("action_register", Callable(self, "_on_rig_element_action_register")):
		group.connect("action_register", Callable(self, "_on_rig_element_action_register"))
...
func _initialize_switch(
	...
	if not switch.is_connected("action_register", Callable(self, "_on_rig_element_action_register")):
		switch.connect("action_register", Callable(self, "_on_rig_element_action_register"))
...
func _get_image_list(image_folder_path: String) -> Dictionary:
	...
	var directory = DirAccess.open(image_folder_path)
	if directory == null:
		push_error("Error trying to open folder: %s" % image_folder_path)
		return image_list
...
func _create_remote_transform(
	...
	if _verbose:
		print("| Created RemoteTransform3D for target %s" % [target.name])
