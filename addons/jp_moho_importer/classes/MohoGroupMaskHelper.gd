	...
	for idx in group.layers.size():
		...
		match mask_mode:
			...
			MaskMode.CLEAR_ADD_MASK, MaskMode.CLEAR_ADD_MASK_INVIS:
				...
				push_error(
					"Moho Mask Mode mode %s isn't supported | Sprite2D: %s"
					% [mask_mode_name, sprite.name]
				)
			_:
				push_error("Unknown Mask Mode: %s | Sprite2D: %s" % [mask_mode, sprite.name])
...
func _add_mask(sprite: MohoSprite, light_mask_layer: int, invisible: bool = false) -> void:
	...
	var light2d_mask := PointLight2D.new()
	...
	light2d_mask.blend_mode = Light2D.BLEND_MODE_MIX
...
func _create_mask_texture(og_texture: Texture2D) -> Texture2D:
	var og_image : Image = og_texture.get_image()
	...
	false # og_image.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	false # mask_image.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	...
	false # og_image.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	false # mask_image.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
