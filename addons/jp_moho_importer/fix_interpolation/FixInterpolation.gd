...
var animation_tree: AnimationTree = null: set = _set_animation_tree
...
func _has_track(animation: Animation, track_path: NodePath) -> bool:
	return animation.find_track(track_path, Animation.TYPE_VALUE) >= 0


func _add_checkbox(anim_name: String) -> void:
	var check_box := CheckBox.new()
	check_box.text = anim_name
	check_box.connect("toggled", Callable(self, "_on_checkbox_toggled").bind(anim_name))
...
class AnimationData:
	...
	func save_animation() -> void:
		if not animation.resource_path.is_empty():
...
class TrackData:
	...
	func _update_from_path() -> void:
		track_idx = animation.find_track(track_path, Animation.TYPE_VALUE)
		track_type = animation.track_get_type(track_idx)
