...
var ik: Skeleton2DIK = null: set = _set_ik
var ik_bake_helper: IKBakeHelper = null: set = _set_ik_bake_helper
...
func bake() -> void:
	...
	if not _current_animation.resource_path.is_empty():
		ResourceSaver.save(_current_animation, _current_animation.resource_path)
		
		if _current_animation.resource_path.ends_with(".tres"):
			var resource_path := _current_animation.resource_path.replace(".tres", ".res")
			ResourceSaver.save(_current_animation, resource_path)
...
func open() -> void:
	...
	_force_loop_button.button_pressed = _force_loop
...
func _get_track_for(chain_node: Node2D, should_create_new: bool = true) -> int:
	...
	var track_idx := _current_animation.find_track(track_node_path, Animation.TYPE_VALUE)
...
func _create_helper() -> void:
	var helper : IKBakeHelper = _root_scene.find_child(HELPER_PATH)
	if helper == null:
