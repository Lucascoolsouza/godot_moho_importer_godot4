...
@export var scene_paths : Array # (Array, String, FILE, "*.tscn")
...
func merge() -> void:
	...
	for scene_path in scene_paths:
		...
		var scene_merge_from : Node2D = packed_scene.instantiate()
	...
	ResourceSaver.save(final_scene_packed, final_scene_path)
...
static func set_owner_recursive(node: Node, p_owner: Node = null) -> void:
	...
	if node.owner == null or node.owner != p_owner:
		node.owner = p_owner
	var is_not_packed_scene = node.filename.is_empty()
